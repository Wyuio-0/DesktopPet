    ...
    companion object {
        ...
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            try {
                ...
                views.setImageViewResource(R.id.widget_avatar, currentSkin.defaultResId)
                ...
                if (ScheduleManager.courses.isEmpty()) {
                    ...
                    if (todayCourses.isEmpty()) {
                        ...
                        if (tomorrowCourses.isNotEmpty()) {
                            ...
                            val tmRoomStr = firstTomorrow.room.ifEmpty { "待定地点" }
                            views.setTextViewText(R.id.widget_empty_title, "📅 今日全天无排课")
                            views.setTextViewText(R.id.widget_empty_desc, "明日首节 $tmStartStr《${firstTomorrow.name}》@$tmRoomStr")
                        } else {
                            ...
                            if (nextExam != null) {
                                ...
                                val cdStr = if (days > 0) "剩 $days 天 $hours 时" else "仅剩 $hours 时"
                                views.setTextViewText(R.id.widget_empty_title, "🎯 备战期末 · $cdStr")
                                val loc = if (nextExam.location.isNotEmpty()) "@${nextExam.location}" else ""
                        ...
                        if (nextCourse == null) {
                            ...
                            if (tomorrowCourses.isNotEmpty()) {
                                ...
                                if (nextExam != null) {
                                    val (days, hours, _) = ExamManager.getCountdownParts(nextExam.examTimeMillis)
                                    val cdStr = if (days > 0) "剩 $days 天" else "仅剩 $hours 小时"
                                    views.setTextViewText(R.id.widget_empty_title, "🎉 今日已结课 · 备战期末 ($cdStr)")
                                    val loc = if (nextExam.location.isNotEmpty()) "@${nextExam.location}" else ""
                                    views.setTextViewText(R.id.widget_empty_desc, "下场《${nextExam.title}》$loc (座号: ${nextExam.seatNumber.ifEmpty { "待定" }})")
                                } else {
                                    views.setTextViewText(R.id.widget_empty_title, "🎉 今日所有课程已结束")
                            ...
                            val statusTag = when {
                                curTotalMin in stMin until etMin -> "🔴 正在上课"
                                stMin - curTotalMin in 1..60 -> "⚡ 距上课剩 ${stMin - curTotalMin} 分钟"
                                else -> "▶ 下一节"
                            }

                            views.setTextViewText(R.id.widget_next_tag, statusTag)
                            views.setTextViewText(R.id.widget_next_section, "第 ${nextCourse.secStart}-${nextCourse.secEnd} 节 · $startTimeStr")
