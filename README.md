...
### 🔍 如何抓取课表数据（F12 抓包保姆级教程，支持强智等各大高校教务）

1. **电脑登录教务系统进入课表**：电脑浏览器（推荐 Edge/Chrome）登录学校教务系统，点击进入「个人课表查询」或「学生课表」页面，选好当前学年与学期（如 2026-2027-1），此时先不要点查询。
2. **按 F12 开启开发者工具**：按键盘 `F12`（部分笔记本按 `Fn+F12`，或鼠标右键页面空白处选择「检查」）。在弹出的控制台顶部，切换到 **「Network / 网络」** 标签栏。
3. **点击查询并筛选特殊请求**：保持 F12 网络面板开启，在网页上点击「查询」按钮触发加载。在 Network 顶部的搜索筛选框（Filter）中输入关键字：`xskbcx` 或 `kbList`。请求列表中会出现名为 `xskbcx_cxXsksxxlist`（或包含 `kbcx`）的网络请求。
4. **核对特殊特征值并完整复制**：点击该请求，在右侧面板切到 **「Response（响应）」**（或 Preview 预览）。核对返回内容里必须包含 `"kbList"` 数组（含 `"kcmc"` 课程名、`"cdmc"` 教室、`"zcd"` 周次、`"jc"` 节次等核心字段）。在响应内容区域右键点击 **「Copy response（复制响应内容）」** 或全选复制完整 JSON 数据（以 `{` 开头、以 `}` 结尾）。
5. **保存或发送到手机**：
   - 💻 电脑端：可保存为 `.json` 文件（例如 `kebiao.json`）；
   - 📱 手机端：直接将文本内容通过微信文件传输助手/QQ发到手机剪贴板中。
